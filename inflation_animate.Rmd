...
`r firstup(glosa1)` anoto `r ll_11` mensuales en `r n11` de sus `r n1` clases. La más importante fue `r glosa11`(`r var_m_11`%) que incidió `r inc_m_11`pp., mientras que `r glosa12`(`r var_m_12`%) contribuyó `r inc_m_12`pp.

+ División `r d2` de `r glosa2`

`r firstup(glosa2)` consignó `r ll_21` mensuales en `r n21` de sus `r n2` clases. La más importante fue `r glosa21`(`r var_m_21`%) que aportó `r inc_m_21`pp., seguida de `r glosa22`(`r var_m_22`%) con `r inc_m_22`pp.

+ División `r d3` de `r glosa3`

`r firstup(glosa3)` registró `r ll_31` mensuales en `r n31` de sus `r n3` clases. La más importante fue `r glosa31`(`r var_m_31`%) que aportó `r inc_m_31`pp., seguida de `r glosa32`(`r var_m_32`%) con `r inc_m_32`pp.
...
En `r mm1` presentó un `r ll_prod1` mensual de `r var_prod1`%, aportando `r inc_prod1`pp. a la variacion del indicador general. Acumuló `r acu_prod1`% al tercer mes del año y `r doc_prod1`% a doce meses.

+ `r firstup(tolower(prod2))`

Anotó un `r ll_prod2` mensual de `r var_prod2`%, con una incidencia de `r inc_prod2`pp., y variaciones de `r acu_prod2`% y de `r doc_prod2`% a doce meses.

+ `r firstup(tolower(prod3))`

Registró un `r ll_prod3` mensual de `r var_prod3`% con `r inc_prod3`pp., acumulando `r acu_prod3`% en lo que va del año y `r doc_prod3`% a doce meses.

+ `r firstup(tolower(prod4))`

Consignó un `r ll_prod4` mensual de `r var_prod4`%, con `r inc_prod4`pp., acumulando `r acu_prod4`% en lo que va del año y `r doc_prod4`% a doce meses.

+ `r firstup(tolower(prod5))`

Consigno un `r ll_prod5` mensual de `r var_prod5`%, con una incidencia de `r inc_prod5`pp., registrando una variación de `r acu_prod5`% en lo que va del año y de `r doc_prod5`% a doce meses

+ `r firstup(tolower(prod6))`

Registró una `r ll_prod6` mensual de `r var_prod6`%, con `r inc_prod6`pp., acumulando `r acu_prod6`% en lo que va del año y `r doc_prod6`% a doce meses.
