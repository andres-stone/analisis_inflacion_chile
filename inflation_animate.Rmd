...
pi <- read_excel(paste0("../inflation_animate/datos/",pi)) %>%
  janitor::clean_names()
pi <- pi[-1,c(1:2)]
colnames(pi) <- c('Periodo', 'IPC')

# Tasa de Politica Monetaria Mensual
tpm <- read_excel(paste0("../inflation_animate/datos/",tpm)) %>%
  janitor::clean_names()
tpm <- tpm[-1,c(1:2)]
...
Para `r mm1`la inflación anual del IPC `r ll_pi` `r l_pi`% interanualmente, mientras que la tasa de interés de política monetaria se ubica en `r l_tpm`% para el mes mencionado.
